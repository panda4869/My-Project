...
def min_max_scale(train_data, test_data):
    sc = MinMaxScaler()
    scale_list = ['first_MT_quality','credit_amount','gift_amount','signup_to_book_interval','income_level','num_hvc']
...
if __name__=="__main__":
    ...
    parser.add_argument("--s3-save", action="store_true")
    ...
    try:
        if args.trainbasemodel:
            ...
            save_model(clf_xgb, args.s3_save)
            clf_xgb.fit(np.array(X_train),np.array(y_train))
            X_train_new,X_test_new=create_new_features(clf_xgb,X_train=X_train,X_test=X_test)
    except Exception as e:
    ...
    if args.trainmodel:
        ...
        save_model(clf_LR, args.s3_save)
