...
where ((date(f2.local_hour2) >=date(CONVERT_TZ(now(),'UTC','US/Pacific')-interval weekday(CONVERT_TZ(now(),'UTC','US/Pacific')) day-interval 1 week) and date(f2.local_hour2)<date(CONVERT_TZ(now(),'UTC','US/Pacific')-interval weekday(CONVERT_TZ(now(),'UTC','US/Pacific')) day) ) ) and f1.city_id is not null
