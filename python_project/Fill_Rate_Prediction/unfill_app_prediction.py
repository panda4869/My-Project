...
if __name__=='__main__':
    ...
    data_fill_dummies_train.drop(['id'],axis=1,inplace=True)
    ### preprocessing testing data
    data_fill_test=pd.read_csv(test_filename)

    data_fill_dummies_test=cat_preprocessing(data_fill_test)
    ...
    for city_id in city_list:
        ...
        data_fill_dummies_city_test=data_fill_dummies_test[data_fill_dummies_test.city_id==city_id]
        X_Test=data_fill_dummies_city_test.loc[:,predictors]
        X_Train_add_geo,X_Test_add_geo=geodata_generate(X_Train,X_Test)
        X_Train_add_geo_trans=math_transform(X_Train_add_geo)
        X_Test_add_geo_trans=math_transform(X_Test_add_geo)
        X_Train_scaled,X_Test_scaled=scaler(X_Train_add_geo_trans,X_Test_add_geo_trans)
        ...
        if args[0]=='--trainmodel':
            ...
            best_model=load_model(city_id)
            best_threshold=load_threshold(city_id)
            if n==len(city_list):
                del args[0]
        else: 
            print('Usage: python3 xx.py [--trainmodel/loadmodel] [--predict]')
            sys.exit(1)

        if args[1]:=='--predict':
            if best_model:
                best_model.fit(np.array(X_Train),np.array(y_Train))
                pred=np.where(best_model.predict_proba>best_threshold,1,0)
                if n==1:
                    output=pd.DataFrame({'ID':data_fill_dummies_city_test.id,'Unfilled_Probability',pred,'City',city_id})
                current=pd.DataFrame({'ID':data_fill_dummies_city_test.id,'Unfilled_Probability',pred,'City',city_id})
        ...
        n+=1
            if n==length(city_list):
                save_path=os.getcwd()+'/prediction.csv'
                if os.path.exists(save_path):
                    os.remove(save_path)
